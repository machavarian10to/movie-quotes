<x-layout>
    <div class="flex min-h-full flex-col justify-center py-12 sm:px-6 lg:px-8">
        <x-title name="{{__('texts.publish_new_movie')}}" />
        ...
        <div class=" sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                ...
                <form action="{{ route('admin.movies_store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" >
                    ...
                    <x-input name="name_en" title="{{__('texts.movie_name_english')}}"/>
                    <x-error type="name_en" />

                    <x-input name="name_ka" title="{{__('texts.movie_name_georgian')}}"/>
                    <x-error type="name_ka" />

                    <x-input name="slug" title="{{__('texts.slug')}}"/>
                    <x-error type="slug" />
